...
User input: $ARGUMENTS (worktree name is required; additional flags are optional)
...
2. Extract the worktree name from `$ARGUMENTS`. The worktree name is the first positional argument (typically a ticket ID like `DEVX-1001`). If no worktree name is provided, exit with an error. Parse any additional flags the user included (e.g., `--dry-run`, `--skip-cmux`, `--skip-workspace`, `--branch develop`).
...
- Full setup (e.g., `/devx:setup-worktree TICKET-1`):

  ```bash
  ${PLUGIN_DIR}/skills/worktree-setup/scripts/setup-worktree.sh TICKET-1
  ```

- With custom branch:

  ```bash
  ${PLUGIN_DIR}/skills/worktree-setup/scripts/setup-worktree.sh TICKET-1 --branch develop
  ```

- Dry-run preview:

  ```bash
  ${PLUGIN_DIR}/skills/worktree-setup/scripts/setup-worktree.sh TICKET-1 --dry-run
  ```

- Skip cmux terminal setup:

  ```bash
  ${PLUGIN_DIR}/skills/worktree-setup/scripts/setup-worktree.sh TICKET-1 --skip-cmux
  ```

- Skip VS Code workspace update:
  ```bash
  ${PLUGIN_DIR}/skills/worktree-setup/scripts/setup-worktree.sh TICKET-1 --skip-workspace
  ```
...
- **Exit 1 -- Usage error:** Report the missing or invalid argument (e.g., missing worktree name, not inside a git repository). Show the correct syntax.

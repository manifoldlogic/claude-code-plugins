...
**Purpose**: Initialize a new ticket with planning document templates. Uses smart document triage to select relevant documents based on the ticket description.
...
**Syntax**:
```
/sdd:plan-ticket [description or TICKET_ID name] [+doc -doc] [additional instructions]
```

**Parameters**:
- `description` or `TICKET_ID name` (required): Ticket description or explicit ID + name
- `+doc-name` (optional): Force include a document (e.g., `+accessibility`)
- `-doc-name` (optional): Force exclude a document (e.g., `-runbook`)
- `additional instructions` (optional): Extra context for the ticket-planner agent

**Document Triage**:

The command automatically triages which planning documents to generate:
- **Core** (always): analysis.md, architecture.md, plan.md
- **Standard** (default): prd.md, quality-strategy.md, security-review.md
- **Conditional** (keyword-triggered): observability.md, migration-plan.md, accessibility.md, api-contract.md, runbook.md, dependency-audit.md

Override flags adjust the triage result. See the Override Syntax section below.

**Behavior**:
1. Parses description and extracts override flags
2. Runs triage-documents.sh to select relevant documents
3. Displays triage results and asks for developer confirmation
4. Runs scaffold-ticket.sh with triage manifest (selected documents only)
5. Delegates to ticket-planner agent (Sonnet) to fill planning documents
6. Creates README with links to generated documents only
...
**Example 2: With override flags**:
```bash
/sdd:plan-ticket "backend API service" +observability +api-contract -dependency-audit
```

**Example 3: Overrides with planning context**:
```bash
/sdd:plan-ticket "data pipeline" +observability -runbook Focus on reliability and error handling
```

**Example 4: ID-name pattern with overrides**:
```bash
...
Documents to skip:
  - observability, migration-plan, accessibility, api-contract, runbook, dependency-audit

Proceed with these documents? [Yes, proceed / No, cancel]

TICKET CREATED: AUTH_user-authentication-oauth

Documents generated: 6 of 12
Documents skipped: 6

Interactive prompt: "What would you like to do next?"
Options: /sdd:review AUTH, /sdd:create-tasks AUTH, ...
```
...
| Override | Effect |
|----------|--------|
| `+accessibility` | Force include accessibility.md even if no UI keywords in description |
| `-runbook` | Force exclude runbook.md even if deploy keywords are present |
| `+observability +api-contract` | Force include multiple conditional documents |
| `-security-review` | Force exclude a standard document |
| `-analysis` | No effect (core documents cannot be excluded) |
...
**Warning**: "Unknown document override 'foo-bar'"
- **Cause**: Override name does not match any document in the registry
- **Solution**: Check valid document names in document-registry.json

**Fallback**: If triage-documents.sh fails, the command falls back to legacy behavior (generates the original six default documents).
...
/sdd:plan-ticket ID name [+doc -doc] → Create ticket (triage selects documents)
[Planning docs auto-filled]   → ticket-planner fills selected documents

...
1. **Execute Test Suites**: Run the appropriate test commands based on the ticket structure (yarn test, yarn test:only, etc.)
...
2. **Select Appropriate Scope** - For example: 
   - Use `yarn test` for full test suite
   - Use `yarn test:only` for unit tests only
...
- Execution time
- Coverage percentage (if available from test output)
...
- Coverage status (meets thresholds or not)
...
- **Build Required**: If tests require a build step (yarn build), note this in your report
...
- **Coverage Reporting**: Always include coverage data when available in test output
- **Threshold Awareness**: Flag when coverage drops below configured thresholds

...
#
# Analyzes implementation files and test files to produce a
# coverage report showing which functions are tested.
#
...
OUTPUT_FILE="${SCRIPT_DIR}/coverage-report.md"

# Temporary files
IMPL_FUNCS="/tmp/impl_funcs_$$.txt"
TEST_FUNCS="/tmp/test_funcs_$$.txt"

cleanup() {
    rm -f "$IMPL_FUNCS" "$TEST_FUNCS"
}
trap cleanup EXIT

echo "Generating test coverage report..."

# Extract all function names from implementation files
{
    echo "# Functions from orchestrator.sh"
    grep -E "^[a-z_]+\(\)" "${AUTOMATION_DIR}/orchestrator.sh" | sed 's/().*//' || true

    echo "# Functions from lib/common.sh"
    grep -E "^[a-z_]+\(\)" "${AUTOMATION_DIR}/lib/common.sh" | sed 's/().*//' || true

    echo "# Functions from modules"
    for module in "${AUTOMATION_DIR}"/modules/*.sh; do
        echo "# Functions from $(basename "$module")"
        grep -E "^[a-z_]+\(\)" "$module" | sed 's/().*//' || true
    done
} > "$IMPL_FUNCS"
...
# Extract all test function names
{
    for test_file in "${SCRIPT_DIR}"/test_*.sh; do
        echo "# Tests from $(basename "$test_file")"
        grep -E "^test_[a-z_]+\(\)" "$test_file" | sed 's/().*//' || true
    done
} > "$TEST_FUNCS"

# Count totals
total_impl_funcs=$(grep -E "^[a-z_]+" "$IMPL_FUNCS" | wc -l)
total_test_funcs=$(grep -E "^test_" "$TEST_FUNCS" | wc -l)
...
- **Implementation Functions**: $total_impl_funcs
...
echo
echo "Summary:"
echo "  Implementation functions: $total_impl_funcs"

...
# Ticket Boundary Framework for Agent-Based Development

## The Stable Context Triangle

At the core of effective agent-based development is defining the right ticket boundaries. A well-bounded ticket creates a **stable context** where AI agents can operate effectively without confusion or inconsistency.
...
**The Golden Rule:** All external interfaces must remain stable throughout the ticket lifecycle.

**What This Means:**
- APIs your ticket calls won't change mid-development
...
### 2. Context Coherence 📦
**The Memory Rule:** The entire ticket must fit within an agent's working memory.

**What This Means:**
- The complete ticket can be explained in 2-3 paragraphs
...
**How to Verify:**
- Write a ticket summary in <500 words
- List all domain entities (should be <20)
- Map file dependencies (should cluster tightly)
- Test: Could a new developer understand the ticket from your docs?
...
- Binary pass/fail for the entire ticket
...
- A script could determine ticket completion
...
### Domain Unity
The ticket should operate within a single domain language. Mixing multiple domains (e.g., "billing" + "user management" + "reporting") creates context confusion.

### Independent Value
The ticket should deliver standalone value, not require other tickets to be useful. This ensures clear scoping and prevents dependency cascades.

---

## Ticket Boundary Patterns
...
### The New Agent Test
*"If I gave this ticket to a fresh agent halfway through, could it understand what's being built?"*
- ✅ Yes → Good boundary
...
*"Can I fit the essential ticket context in a single conversation?"*
- ✅ Yes → Good boundary
- ❌ No → Too large, split it

### The Script Completion Test
*"Could a script determine if this ticket is complete?"*
- ✅ Yes → Good boundary
...
context_coherence:
  ☐ Ticket explainable in <500 words
...
| Context too large | Medium - agent confusion | Split into sub-tickets |
...
1. Write ticket summary (<500 words)
...
4. Each phase becomes its own ticket

### If Your Ticket Is Too Small
...
2. Delay ticket until interfaces stabilize
...
1. **Interface stability is non-negotiable.** It's better to delay a ticket than start with unstable interfaces.
...
3. **Testable completion prevents drift.** Without clear verification, tickets never truly complete.
...
5. **Start strict, then relax.** It's easier to expand boundaries than to contract them mid-ticket.
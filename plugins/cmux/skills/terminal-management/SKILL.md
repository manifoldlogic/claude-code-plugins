...
The terminal-management skill enables Claude Code to control cmux terminals on a macOS host from inside a devcontainer. Because cmux CLI does not work from SSH remote sessions (Issue #373), all commands must be proxied through SSH to the host using the full binary path. The workspace is the primary operational unit -- each workspace appears as a vertical tab in the cmux sidebar and contains one or more panes and surfaces.
...
The `cmux-ssh.sh` wrapper script handles SSH connection details, input validation, and argument escaping automatically. It is the recommended way to run cmux commands from the devcontainer.

**Location:** `plugins/cmux/skills/terminal-management/scripts/cmux-ssh.sh`
...
| Request | Action |
|---------|--------|
| "Open a new workspace/tab" | `cmux-ssh.sh new-workspace` (note the `workspace:N` ID returned) |
| "Send this command to another session" | `cmux-ssh.sh list-workspaces`, then two-step send to target `workspace:N` |
| "List my open workspaces/terminals" | `cmux-ssh.sh list-workspaces` |
...
# Or use cmux-ssh.sh wrapper:
cmux-ssh.sh new-workspace
cmux-ssh.sh rename-workspace --workspace workspace:2 'My Work'
```
...
cmux-ssh.sh new-workspace
sleep 0.5
cmux-ssh.sh rename-workspace --workspace workspace:2 'Build'
sleep 0.5
cmux-ssh.sh send --workspace workspace:2 'make build'
cmux-ssh.sh send-key --workspace workspace:2 enter
```
